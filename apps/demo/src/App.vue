...
const backends = [
  ...
  { label: 'C#', value: import.meta.env.VITE_BACKEND_CSHARP, disabled: import.meta.env.VITE_CSHARP_DISABLED === 'true' },
]
// 过滤出在线后端，取第一个作为默认
const onlineBackends = backends.filter(b => !b.disabled)

// ── URL ?lang= 契约：website 各语言演示卡片携带 ?lang=xx，自动定位对应 Tab ──
const LANG_MAP = { java: 'VITE_BACKEND_JAVA', go: 'VITE_BACKEND_GO', python: 'VITE_BACKEND_PYTHON', node: 'VITE_BACKEND_NODE', php: 'VITE_BACKEND_PHP', rust: 'VITE_BACKEND_RUST', moon: 'VITE_BACKEND_MOON', csharp: 'VITE_BACKEND_CSHARP' }
