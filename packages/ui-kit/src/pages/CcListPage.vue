<template>
  <div class="jf-page">
    <h2 class="jf-page-title">
      我的抄送
      <button v-if="can(['wf:processInstance:createCCInstance']) && detailInstanceId" class="jf-btn jf-btn--ghost jf-btn--sm" @click="ccVisible = true">手动抄送</button>
    </h2>
    <div v-if="loading" class="jf-loading">加载中...</div>
    <template v-else>
      <table v-if="rows.length" class="jf-table">
        <thead><tr><th>流程</th><th>状态</th><th>发起人</th><th>时间</th><th>操作</th></tr></thead>
        <tbody>
          <tr v-for="i in rows" :key="i.id">
            ...
            <td><button class="jf-btn jf-btn--ghost jf-btn--sm" @click="openDetail(i.id)">详情 →</button></td>
          </tr>
        </tbody>
      </table>
...
async function reload() {
  loading.value = true
  try {
    const r = await api.processInstance.ccList({ pageNum: pageNum.value, pageSize })
    rows.value = r.rows
...
async function doCc() {
  ...
  ccSaving.value = true
  try {
    await api.processInstance.createCCInstance(detailInstanceId.value, ids)
